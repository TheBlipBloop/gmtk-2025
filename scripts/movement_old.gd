...
@export var base_drag : float = 5;
@export var base_angular_drag : float = 51;

@export var wheel_heading : Vector3;
...
func _physics_process(delta: float) -> void:
	...
	if not m_input_boosting:
		linear_velocity = linear_velocity.move_toward(Vector3.ZERO, delta * base_drag);
	if abs(m_input_rotation) < 0.1:
		angular_velocity = angular_velocity.move_toward(Vector3.ZERO, delta * base_angular_drag);
